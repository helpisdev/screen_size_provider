...
Similarly, a getter that returns a `Future` should not kick-off the work represented by the future, since getters appear idempotent and side-effect free. Instead, the work should be started from a method or constructor, and the getter should just return the pre-existing `Future`.
...
Feel free to be conservative in what you consider "sufficient quality". It's ok to use `//` even if you have multiple paragraphs of documentation; that's a sign that we should carefully review the documentation when making the code public.
...
The `<id>` should be a unique identifier that is of the form `screen_size_provider.library.Class.member[.optionalDescription]`.
For example:

```dart
// GOOD:
/// {@template screen_size_provider.rendering.Layer.findAnnotations.aboutAnnotations}
